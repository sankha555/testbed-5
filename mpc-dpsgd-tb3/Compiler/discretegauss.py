...
def sample_dgauss_stddev(sigma, rng=None):
    if rng is None:
        rng = random.SystemRandom()
    ...
    while True:
        candidate = sample_dlaplace(t, rng = rng)
        bias = ((abs(candidate)-sigma*sigma/t)**2)/(2*sigma*sigma)
        if sample_bernoulli_exp(bias, rng) == 1:
            return candidate
...
"""
from Compiler.types import sfix, regint
from Compiler.library import *
def scaled_noise_sample(std=16, scale=16, i=0, j=0):
    var = std*std*(1 << 2*scale)
    eta = sample_dgauss(var)/(1 << scale)
    return eta

def get_noise_vector(std=0, n=1, u=0):
    noise = [0]*n
    for i in range(n):
        noise[i] = scaled_noise_sample(std=std, scale=16)
    # print_ln("NOISE_KAMATH = %s", noise)
    return noise
